import { BLOCK_TRIAL, TIME_LIMIT, TIMEOUT_SETTING } from './config.js';
...
const update = { // 進捗更新
    ...
    CounterColor() {
        ...
        const percentage = (state.currentBlockTrial / (BLOCK_TRIAL - 1)) * 100;
        counterCard.style.background = `linear-gradient(to top, rgb(0, 140, 255) ${percentage}%, rgb(255, 190, 70) ${percentage}%)`;

        state.currentBlockTrial++;
        if (state.currentBlockTrial >= BLOCK_TRIAL) {
            state.currentBlockTrial = 0;
        }
    },
    ProgressBoxes() {
        // id管理しなくても？
        ...
        const completedBoxes = Math.floor(state.currentQuestionIndex / BLOCK_TRIAL);
